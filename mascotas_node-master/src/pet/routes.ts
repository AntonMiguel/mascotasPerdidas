...
export function initModule(app: express.Express) {
  ...
  app
    ...
    .put(onlyLoggedIn, addMessage);

  app
    .route("/v1/lostPets")
    .get(onlyLoggedIn, findLostPets)
}


/**
 * @api {get} /v1/lostPets Listar Mascotas perdidas
 * @apiName Listar Mascotas perdidas
 * @apiGroup Mascotas
 *
 ...
 * @apiUse OtherErrors
 */
async function findLostPets(req: ISessionRequest, res: express.Response) {
  const result = await service.findLostPets();
...
async function readById(req: ISessionRequest, res: express.Response) {
  ...
  res.json({
    ...
    locationMessages: result.locationMessages
  });
}
...
/**
 * @api {put} /v1/pet/:petId Agregar mensaje Mascota
 * @apiName Agregar mensaje Mascota
 * @apiGroup Mascotas
 *
 * @apiDescription Agrega un mensaje a una mascota perdida.
 *
 * @apiUse IMascotaResponse
 *
 * @apiUse AuthHeader
 ...
 * @apiUse OtherErrors
 */
async function addMessage(req: ISessionRequest, res: express.Response) {
  await service.addMessage (req.params.petId, req.body.message, req.body.locationStatus);
  res.send();
}