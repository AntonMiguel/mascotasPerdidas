...
export async function loadLostPets(): Promise<Pet[]> {
    try {
        const res = (await axios.get(environment.backendUrl + "/v1/lostPets")).data as Pet[]
...
export async function addLostPetMessage(id:string, message:string, locationStatus:string): Promise<void> {
    try {
        await axios.put(environment.backendUrl + "/v1/pet/" + id, {locationStatus:locationStatus, message: message})
        return Promise.resolve()
    } catch (err) {
        return Promise.reject(err)
    }
